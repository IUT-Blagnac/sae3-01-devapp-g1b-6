<?php
	...
	$regex[0] = "#^[A-Z][a-zA-Z]+$#";
	$regex[1] = "#^([a-z]|[0-9]|\.|-|_)+@([a-z]|[0-9]|\.|-|_)+\.([a-z]|[0-9]){2,3}$#";
	$regex[2] = "#^[0][1-9]{1}[0-9]{8}$#";
	$regex[3] = "#^\S*(?=\S{8,})(?=\S*[a-z])(?=\S*[A-Z])(?=\S*[\d])\S*$#";
	...
	if (isset($_POST['valider']) && isset($_POST['email']) 
		...
		if(!preg_match($regex[0],$_POST['nom'])){
			...
			$req2 = "INSERT INTO Client(idclient,nomclient,prenomclient,telclient,emailclient,mdpclient) VALUES(idClient.NEXTVAL,:pnom,:pprenom,:ptel,:pemail,:pmdp)";
			...
			if (!$result) {
				$e = oci_error($insert);  // on récupère l'exception liée au pb d'execution de la requete (violation PK par exemple)
