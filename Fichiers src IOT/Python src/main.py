import paho.mqtt.client as mqtt, json, yaml, os, sys, time, signal
...
#méthode qui se lance à chaque fin de signal ou interception
def write(signum, code):
    ...
    #vérifications des données par rapport aux seuils max définis dans le fichier de configuration 
...
#on lance la méthode d'écriture lorsque le signal alarm est reçu
signal.signal(signal.SIGALRM, write)
signal.alarm(frequence)

#on boucle à l'infini le programme ci-dessus
client.loop_forever()