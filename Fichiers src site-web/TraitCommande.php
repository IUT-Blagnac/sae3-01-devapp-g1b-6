...
//----------------------------------------------------------//
		...
		$req = "INSERT INTO COMMANDE VALUES (SEQIDCOMMANDE.NEXTVAL, :pID_CLIENT, :pPRIX, :pCARTE, :pCCV, :pDATE)";
...
//-----------------------------------------------------------------//
		...
		$reqId = "SELECT * FROM COMMANDE WHERE IDCLIENT = :pID_CLIENT AND PRIXTOTAL = :pPRIX AND NUMEROCB = :pCARTE AND CRYPTOGRAMMECB = :pCCV AND DATEEXPIRATIONCB = :pDATE";
		...
		while (($articles = oci_fetch_assoc($req4)) != false) { 
			...
			oci_bind_by_name($insert, ":pID_COMMANDE", $idCommande);
			...
			//on met a jour le stock de chaque article
